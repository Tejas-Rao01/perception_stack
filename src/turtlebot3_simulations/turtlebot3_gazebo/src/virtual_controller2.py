...
class LeaderController():
    ...
    def update_leader_state(self, dt):
        ...
        with open('leader_state.txt', 'a') as f:
                f.write(f"{x} {y} \n")
    ...
    def publisher(self):
        ...
        #print(f"thetad_dot before: {thetad_dot}")
        D = np.sqrt(ex ** 2 + ey ** 2)
        ...
        pub_msg.pose.pose.orientation.z = self.theta
        ...
        print(f"publishing, {t_curr}, {x_t}, {y_t}")
        """
    ...
    def get_traj_point(self, t, T_sim):
        x_t = 5 * np.cos(2 * np.pi * t / T_sim) #(5 + np.sin(6 * np.pi * t / T_sim)) * np.cos(2 * np.pi * t / T_sim)
        y_t = 5 * np.sin(2 * np.pi * t / T_sim)  #(5 + np.sin(6 * np.pi * t / T_sim)) * np.sin(2 * np.pi * t / T_sim)
        return x_t, y_t
